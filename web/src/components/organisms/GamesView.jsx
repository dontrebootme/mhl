...
import { fetchGames, fetchDivisions } from '../../services/data';
import { Card, Select } from '../atoms';

const GamesView = ({ seasonId }) => {
    ...
    const loadDivisions = async () => {
      try {
        ...
        if (mounted) {
          ...
          if (data.length > 0) {
            setSelectedDivision(data[0].value);
          }
        }
      } catch (err) {
  ...
  // Load games when division changes
  useEffect(() => {
    if (!selectedDivision) {
      setGames([]);
      setLoading(false);
    ...
    const loadGames = async () => {
      ...
      setError(null);
      try {
        const data = await fetchGames(seasonId, selectedDivision);
        if (mounted) {
          setGames(data);
        }
      } catch (err) {
      ...
      <Card>
        ...
        {!loading && !error && games.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            <p>No games available for this division.</p>
          </div>
        )}
